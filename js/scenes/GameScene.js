export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        // --- ĐẢO ĐẤT BAY ---
        // 1. CÁC THÔNG SỐ ĐỂ BÀ DỄ DÀNG CHỈNH SỬA:
        let daoDat_X = width / 2;    // CHỈNH TRÁI/PHẢI (width / 2 là ở giữa màn hình. Giảm số này đảo qua trái, tăng đảo qua phải)
        let daoDat_Y = 400;         // CHỈNH LÊN/XUỐNG (Số âm càng lớn thì đảo càng bay cao lên phía đỉnh núi)
        let daoDat_Scale = 0.15;      // CHỈNH TO/NHỎ (0.5 là thu nhỏ 50%. Tăng lên 1, 1.5... để làm đảo to hơn)

        // 2. Thêm đảo đất vào Scene
        // setDepth(0.5) giúp đảo nằm đè lên bầu trời (depth 0) nhưng vẫn xếp sau các vật thể khác trên mặt đất (depth 1)
        this.daodat = this.add.image(daoDat_X, daoDat_Y, 'daodat').setDepth(0.5).setOrigin(0.5);
        this.daodat.setScale(daoDat_Scale);

        // 3. Hiệu ứng "bay lơ lửng" (Nhấp nhô nhẹ nhàng)
        this.tweens.add({
            targets: this.daodat,
            y: daoDat_Y - 30,          // Bay nhô lên thêm 30 pixel
            duration: 2500,            // Thời gian 1 lần nhô lên (2.5 giây)
            yoyo: true,                // Tự động hạ xuống lại
            repeat: -1,                // Lặp lại mãi mãi
            ease: 'Sine.easeInOut'     // Giúp chuyển động lơ lửng mượt mà hơn
        });
        
        // (Tùy chọn) Parallax: Nếu bà muốn đảo đất cuộn tốc độ khác với camera khi vuốt
        // this.daodat.setScrollFactor(0.8, 0.8);
        ...
        let gocScaleCaTre = 0.03; // TĂNG/GIẢM KÍCH THƯỚC Ở ĐÂY (Cũ là 0.015)
        this.catre.setScale(gocScaleCaTre); 
        ...
        this.tweens.add({
            targets: this.catre,
            scaleX: gocScaleCaTre * 0.95,  // Tự động ép ngang theo biến mới
            scaleY: gocScaleCaTre * 1.05,  // Tự động phình dọc theo biến mới
            duration: 250,
            yoyo: true,
            repeat: -1,
            ease: 'Sine.easeInOut'
        ...
        let gocScaleCaVo = 0.03; // TĂNG/GIẢM KÍCH THƯỚC Ở ĐÂY (Cũ là 0.015)
        this.cavo.setScale(gocScaleCaVo); 
        ...
        this.tweens.add({ targets: this.cavo, scaleX: gocScaleCaVo * 0.95, scaleY: gocScaleCaVo * 1.05, duration: 280, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
        ...
        let gocScaleCaTaiTuong = 0.03; // TĂNG/GIẢM KÍCH THƯỚC Ở ĐÂY (Cũ là 0.015)
        this.cataituong.setScale(gocScaleCaTaiTuong); 
        ...
        this.tweens.add({ targets: this.cataituong, scaleX: gocScaleCaTaiTuong * 0.95, scaleY: gocScaleCaTaiTuong * 1.05, duration: 320, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });

        this.physics.add.collider(this.cataituong, [cauCaTra, thanhCauHitbox]);
        this.physics.add.collider(this.cataituong, walls);

        // --- CÁ LAU KÍNH BƠI LỘI ---
        this.calaukinh = this.physics.add.sprite(pondX - 30, pondY + 10, 'calaukinh', 0);
        this.calaukinh.setDepth(2.5); 
        
        let gocScaleCaLauKinh = 0.03; // Kích thước bằng với các cá khác
        this.calaukinh.setScale(gocScaleCaLauKinh); 
        
        this.calaukinh.body.setCircle(this.calaukinh.width / 2); 
        this.calaukinh.setBounce(1, 1); 
        this.calaukinh.setDrag(0);      
        
        // Tốc độ bơi khác đi một chút để không bơi đè lên cá khác
        this.calaukinh.setVelocity(-8, 12); 

        // Hiệu ứng lắc lư và thở cho Cá Lau Kính
        this.tweens.add({ targets: this.calaukinh, angle: { from: -3, to: 3 }, duration: 750, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
        this.tweens.add({ targets: this.calaukinh, scaleX: gocScaleCaLauKinh * 0.95, scaleY: gocScaleCaLauKinh * 1.05, duration: 300, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });

        this.physics.add.collider(this.calaukinh, [cauCaTra, thanhCauHitbox]);
        this.physics.add.collider(this.calaukinh, walls);

        // CHO CÁC CON CÁ TỰ VA CHẠM (DỘI VÀO NHAU) - Đã thêm cá lau kính vào mảng
        this.physics.add.collider([this.catre, this.cavo, this.cataituong, this.calaukinh]);
        ...
        // --- NHÀ PET ---
        // 1. Chỉnh kích thước (To/Nhỏ): 1 là giữ nguyên, 0.5 là thu nhỏ một nửa, 1.5 là gấp rưỡi
        let scaleNhaPet = 0.11; 
        
        // 2. Chỉnh vị trí TRÁI/PHẢI (Trục X)
        // Dùng width * số thập phân để canh theo màn hình (VD: width * 0.8 là nằm gần bên phải)
        let vitriX = width * 0.92; 
        
        // 3. Chỉnh vị trí LÊN/XUỐNG (Trục Y)
        // Cách mặt đất một khoảng (VD: height - 100). Bạn có thể cộng trừ số này để đẩy lên/xuống
        let vitriY = height - 420; 

        // Thêm nhà pet vào Scene
        this.nhaPet = this.add.image(vitriX, vitriY, 'nhapet')
            .setOrigin(0.5, 1) // Đặt tâm ở giữa mép dưới của ảnh để dễ dàng đặt chạm mặt đất
            .setScale(scaleNhaPet)
            .setDepth(2); // Chỉnh z-index để nhà pet hiện rõ trên nền (mặt đất thường là depth 1)

        // Lưu nhà pet vào một thuộc tính của class để sau này dễ dàng thao tác (mua pet, chứa pet)
        // Ví dụ: tính năng click vào nhà pet để mở UI
        this.nhaPet.setInteractive();
        this.nhaPet.on('pointerdown', () => {
            console.log("Đã click vào Nhà Pet!");
            // Sau này bạn có thể gọi hàm mở bảng mua pet ở đây
        });
        ...
        for (let i = 0; i < soDotTre; i++) {
            ...
            if (i >= 20 && (i - 20) % 4 === 0 && i <= 54) {
                // --- CHỈNH TRÁI / PHẢI ---
                let cloudX = (width / 2) - bambooX - 10;
                // --- CHỈNH LÊN / XUỐNG ---
                let cloudY = toaDoY - chieuCaoMotDot + 40;

                // Vẽ mây ra màn hình với tọa độ X, Y vừa chỉnh
                let cloud = this.add.image(cloudX, cloudY, 'tangmay').setOrigin(0.5, 0.5);
    ...
    update() {
        ...
        if (this.catre && this.catre.active) {
            if (this.catre.body.velocity.x > 0) {
                // Đang bơi sang phải -> Lật ảnh sang phải
                this.catre.setFlipX(true); 
            } else if (this.catre.body.velocity.x < 0) {
                // Đang bơi sang trái -> Tắt lật ảnh (trở về gốc)
                this.catre.setFlipX(false); 
            }
        }
        ...
        if (this.cavo && this.cavo.active) {
            if (this.cavo.body.velocity.x > 0) {
                this.cavo.setFlipX(true); 
            } else if (this.cavo.body.velocity.x < 0) {
                this.cavo.setFlipX(false); 
            }
        }
        ...
        if (this.cataituong && this.cataituong.active) {
            if (this.cataituong.body.velocity.x > 0) {
                this.cataituong.setFlipX(true); 
            } else if (this.cataituong.body.velocity.x < 0) {
                this.cataituong.setFlipX(false);
            }
        }

        // --- CÁ LAU KÍNH QUAY ĐẦU ---
        if (this.calaukinh && this.calaukinh.active) {
            if (this.calaukinh.body.velocity.x > 0) {
                // Đang bơi sang phải -> Lật ảnh sang phải
                this.calaukinh.setFlipX(true); 
            } else if (this.calaukinh.body.velocity.x < 0) {
                // Đang bơi sang trái -> Tắt lật ảnh (trở về gốc)
                this.calaukinh.setFlipX(false);
            }
        }

        // --- MÂY BAY DI CHUYỂN ---
        if (this.maybay && this.maybay.active) {
            // Mỗi khung hình, máy bay tiến sang trái 0.3 pixel
            this.maybay.x -= 0.3;
    ...
    caDopMoi(ca) {
        ...
        // 1. Chuyển sang frame 1 (Ảnh cá đang há miệng)
        ca.setFrame(1);
        
        // 2. Dùng bộ đếm thời gian, cho cá há miệng trong nửa giây (500 mili-giây)
        this.time.delayedCall(500, () => {
            // Sau nửa giây, kiểm tra xem cá còn tồn tại không rồi mới khép miệng lại
            if (ca && ca.active) {
                ca.setFrame(0); // Trở về frame 0 (Cá ngậm miệng bơi tiếp)
            }
        });
    }
}
